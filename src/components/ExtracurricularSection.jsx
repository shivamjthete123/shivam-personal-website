...
export default function ExtracurricularSection() {
  ...
  return (
    <section id="extracurricular" aria-labelledby="extracurricular-title" className="border-t border-slate-200 bg-slate-50/70 min-h-0 lg:min-h-[90vh] flex flex-col justify-center scroll-snap-section">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 md:px-8 py-10 sm:py-16 md:py-20" data-reveal-container>
        <SectionHeading
        ...
        <div className="mt-8 sm:mt-10 grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
          {activities.map((act, index) => (
            <article
              ...
              className="flex flex-col justify-between rounded-xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-sm hover:shadow-lg hover:border-amber-500/40 hover:-translate-y-1 transition duration-300 group scroll-snap-section"
            >
              <div>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="text-[11px] sm:text-xs font-semibold text-slate-400">{act.period}</span>
                </div>
                <h3 className="mt-2.5 text-base sm:text-lg font-bold text-slate-950 group-hover:text-amber-900 transition-colors">{act.title}</h3>
                <p className="mt-0.5 text-xs font-semibold text-amber-700">{act.category}</p>
                <p className="mt-2.5 text-xs sm:text-sm leading-6 text-slate-600">{act.description}</p>
              </div>
            </article>
          ))}
        </div>
